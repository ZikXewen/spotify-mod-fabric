...
object AuthServer {
    ...
    fun load() {
        ...
        authServer.start()
        Globals.logger.log(Level.INFO, "Started")
        if(refreshToken != "") refresh()
    }
    private fun refresh() {
        val response = Request.get(Globals.ENDPOINT + "getToken?refreshToken=$refreshToken", TokenResponseClass::class)
        if(response != null && response.accessToken.isNotEmpty()) {
            SpotifyControls.accessToken = response.accessToken
            Globals.logger.log(Level.INFO, "Refreshed Token.")
        } else {
            Globals.logger.log(Level.WARN, "Refreshing Failed")
        }
    }
    private fun login(): Boolean {
        val response = Request.get(Globals.ENDPOINT + "getToken?code=$code", TokenResponseClass::class) ?: return false
        SpotifyControls.accessToken = response.accessToken
        refreshToken = response.refreshToken
        Config.refreshToken.set(refreshToken).save()
        return true
    }
    private fun handle(exchange: HttpExchange) {
        if(exchange.requestMethod.equals("GET")) {
            ...
            Globals.logger.log(Level.INFO, response)
